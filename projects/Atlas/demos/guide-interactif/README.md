...
Dix étapes qui s'adressent au lecteur et l'invitent à agir. Le propos n'est pas
...
3. **51 Ko.** Il s'ouvre instantanément, y compris depuis une page de
   présentation où on le lance par curiosité.

## Les dix étapes

| # | Titre | Notion | Geste proposé |
|---|---|---|---|
| 1 | Bienvenue — ceci est un récit | ce qu'est une scène | faire glisser la carte |
| 2 | Une couleur, une catégorie | symbologie catégorisée | cliquer une classe dans la légende |
| 3 | Une couleur, une mesure | graduation, et le **non classé** | comparer avec l'étape 2 |
| 4 | Filtrer — la carte répond | contrôles exposés | pousser le curseur de valeur |
| 5 | Le volume | extrusion, deux variables | pivoter |
| 6 | Le soleil est daté | ombres à 17 h 30 | — |
| 7 | Neuf heures | la même scène, autre lumière | revenir en arrière, comparer |
| 8 | Des objets, pas des formes | modèles 3D du catalogue | cliquer un îlot |
| 9 | La Terre est ronde | projection globe | dézoomer |
| 10 | Et maintenant, la vôtre | ce que ça veut dire dans Grist | — |
...
```bash
node build-guide.mjs      # -> ilots.geojson, reperes.geojson, scene.json
```
...
- **Carrés de ~75 m.** À 0,0022° la grille faisait 1,1 km et on n'en voyait que
  cinq à l'écran.
- **Étape 8 à plat.** Des repères de 17 m entre des blocs de 6 à 50 m, vus en
  oblique, sont masqués — trois cadrages successifs n'y ont rien changé. Une
  étape isole une notion : celle-ci parle d'objets ponctuels, le volume a eu les
  siennes.
...
Le **filtre temporel** (`type: 'time'`, implémenté mais jamais démontré), les
**tuiles XYZ** en fond, le **relief** — écarté à dessein : sur une couche dont
Atlas ne détient pas les entités, le calage n'a qu'une altitude pour toute la
couche, et une leçon ne doit pas s'appuyer sur une approximation qu'elle
n'explique pas. Le **bâti du fond de carte** également.

Autant de suites possibles, si le format tient à l'usage.
